...
type PutOrPost<T extends Client["POST"] | Client["PUT"]> = (
  endpointName: string,
  ...args: Parameters<T>
) => ReturnType<T>;
...
export class APIClient {
  ...
  private constructor(config: LSPConfig) {
  ...
  updateWithProgram: PutOrPost<Client["POST"] | Client["PUT"]> = async (endpointName, path, init) => {
    if (!this.#currentEndpoints.has(endpointName)) {
      ...
      if (data) {
        ...
        this.#logger.info("Successfully POST'd decision service program: ", data);
      } else if (error) {
  ...
  async updateEndpointInfo() {
    const { data, error } = await this.#client.GET("/functions", {});
    if (data) {
      const endpoints: EndpointName[] = data.map((fn) => fn.function?.name).filter((n): n is string => n !== undefined);
