...
mkCEvalExpr :: ExprF CEvalExpr CST.Lit CST.Decl -> CEvalExpr
mkCEvalExpr = coerce

-- | For wiring up to a non-symbolic evaluator like Simala
newtype CEvalExpr = CEvalExpr (ExprF CEvalExpr CST.Lit CST.Decl)
  deriving newtype (Eq, Show)
  deriving stock (Generic)
  
-- | For symbolic evaluation
newtype SymExpr = MkSymExpr (ExprF (Union SymExpr) SymLit Decl)
  deriving newtype (Eq, Show)
  deriving stock (Generic)
  deriving (Mergeable, ExtractSym, EvalSym) via (Default (ExprF (Union SymExpr) SymLit Decl))
